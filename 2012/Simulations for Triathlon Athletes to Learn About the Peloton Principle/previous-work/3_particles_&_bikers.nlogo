...
[
  ...
  all_collisions  
  runner_x_collisions
  runner_x_collisions_in_precentege
]
...
to setup1 ;create road and runners
  ca
;  set watch-one? false
  ask patches [ setup-road ]
  set manage-view-updates? true
  set view-update-rate 0.2
  set-default-shape runners "circle"
  set runner_size 6  
  make-runners  
  clear-all-plots
  setup-histogram
...
to setup2
  if mouse-down? [
    let candidate min-one-of runners [distancexy mouse-xcor mouse-ycor]
...
to setup3 ;create pareicles
  ...
  ask particles [check-for-runners-collision ] 
...
to go
  ifelse watch-biker [ watch runner 0 ] [ reset-perspective ]
  ...
  if colliding-particle-1 != nobody [ask colliding-particle-1 [check-for-runners-collision]]
...
;    ifelse (watch-one?) [
;      ifelse (who-will-be-watched? > 0) and (who-will-be-watched? <= count runners)[
;        watch runner (who-will-be-watched? - 1)
;        report_runner_x_collisions
;      ]
;      [
;       ; output-write "Must be number between 0 - "runners_number + 1
;        set runner_x_collisions 0
;        set runner_x_collisions_in_precentege 0
;      ]  
;    ]
;    [
;      rp
...
to setup-histogram
  ...
  set-plot-x-range 1 count runners + 1
  set-plot-y-range 0 25
  set-histogram-num-bars count runners
end
  ...
  set plot-counter 1
     ask runners[
       if sum collision-history != 0 [
         ...
         plot [sum collision-history] of runner (plot-counter - 1)
...
;to report_runner_x_collisions
;  ask runner (who-will-be-watched? - 1)[
;      set runner_x_collisions number_of_particles_collided_with
;      if number_of_particles_collided_with != 0 [
;        set runner_x_collisions_in_precentege (number_of_particles_collided_with / all_collisions) * 100
;      ]
;    ]
;end
...
to random-position
  ...
  let runner_radius (runner_size / 2)
  let both_radii (particle_radius + runner_radius)
...
to check-for-runners-collision
...
529
363
step 1:Press \"setup1\" to show path and biker\nstep 2:Press \"setup2\",relocate biker by dragging him and press \"setup2\" again when finished \nstep 3:Press \"setup3\" to locate particles randomly on screen\nstep 4:Press \"go\" to run model
13
0.0
1
