...
to go
  ...
  ;; move bikers
  ask bikers [ jump speed * tick-delta ]
...
Bikers are placed manually on the world and move at constant speed and direction.
...
Currently bikers are only capable of moving in the same direction and at constant speed, maintaining the their relative position to each other on the road. It is possible to give bikers more sophisticated movement behaviour in order to test the effectiveness of more advanced formation and movement tactics.

There are to extensions to this model which allow for more sophisticated biker movement.

* **Particles and Bikers with Movement** adds different movement tactics for bikers, allowing them to change their relative position in pre-defined ways.
* **Particles and Bikers with Flocking** adds the flocking rules found in the **Flocking** model to the bikers, causing them to autonomously group together.
